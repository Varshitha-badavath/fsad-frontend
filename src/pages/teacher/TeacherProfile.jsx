...
import { GraduationCap, Mail, Building, Calendar, Edit3, Save } from 'lucide-react'
...
export default function TeacherProfile() {
  ...
  const fields = [
    ...
    { icon: GraduationCap, label: 'Role',      value: 'Faculty Member / Teacher' },
    { icon: Calendar,     label: 'Joined',     value: user?.joined || '2020-08-01' },
  ]
      ...
      <div className="max-w-xl space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-card p-6 flex items-center gap-5">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-700
            flex items-center justify-center text-white text-2xl font-bold shrink-0 shadow-lg shadow-blue-100">
            {user?.name?.slice(0, 2).toUpperCase()}
          </div>
          <div>
            <h2 className="font-display text-xl font-bold text-slate-900">{user?.name}</h2>
            <p className="text-slate-500 text-sm">{user?.department || 'Computer Science'}</p>
